...
In this article I walk though a method how to efficiently load data from S3 to Snowflake in the first place, and how to integrate this method with DBT using a custom materialization macro.

## Introduction

Currently at my work I was pondering how to efficiently load terrabytes of data stored in S3 to
our new shiny instance of Snowflake with (DBT)[https://getdbt.com].

The data architecture of the company has evolved to an architecture where all of our data ends up in S3 (either directly written by various data ingestion applications or offloaded from our Postgres instance). Having such a "(data lake)[https://en.wikipedia.org/wiki/Data_lake]" as a component of data architecture has been broadly discussed lately, and offers numerous advantages, one of which is the ability to make the data available for analytical queries by loading it to specialized databases like Redshift, Snowflake, Druid etc.
...
That is all we need to load our first model with DBT.
...
and after running a quick test with `dbt run` we can confirm the model was indeed creates with the data loaded from the stage. For small size table that is all we need!

But if the data we are loading is of any significant size, this materialization immediately shows its shortcomings. After the first initial load and a little bit of waiting time, any consequent `dbt run` will try to re-load the whole table. Why? Because DBT's `table` materialization uses CTAS (create table as select) statement, which can be verified by looking at the generated `target/run/<project name>/<model>.sql` file. For the model above it should look like this:
...
For large raw tables what we really want is to detect and load only new files that were created at the stage S3 location (you are not updating historic files in S3 are you?).

## Second attempt - incremental materialization
...
After running this model a second time, you will see that the incremental load takes about the same time as the original full load. Why is that? Because a Snowflake stage is not a table, and `select ... from @my_s3_stage where event_time > ...` will still read every raw file inside the stage, parse it, and then filter it based on `event_time`. That is still a lot of i/o and processing time, especially if Snowflake charges for compute time.
